...
const DEPARTMENTS = ['All', 'Executive', 'Human Resources', 'Product', 'Finance', 'IT'];
...
export default function EmployeesPage() {
  ...
  useEffect(() => {
    ...
    if (!loading && session && !['HR Admin', 'Super Admin'].includes(session.role)) router.push('/restricted');
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Type',
      hideOnMobile: true,
      render: (row: Employee) => <Badge variant="primary">{row.employmentType}</Badge>,
    },
    {
      key: 'startDate',
      header: 'Start Date',
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        action={
          <Button icon={<Plus size={16} />} onClick={() => setShowAddModal(true)} id="add-employee">
            Add Employee
          </Button>
        }
      />
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-0">
              {[
                ...
                { label: 'Employment Type', value: selectedEmployee.employmentType },
                { label: 'Start Date', value: formatDate(selectedEmployee.startDate) },
                { label: 'Base Salary', value: formatCurrency(selectedEmployee.baseSalary) + '/mo' },
