...
export default function DashboardPage() {
    ...
    const fetchAll = async () => {
      ...
      if (["HR Admin", "Super Admin"].includes(session.role)) {
        fetches.push(
          fetch("/api/employees")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.employees = d.data;
            }),
          fetch("/api/payroll")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.payrollRuns = d.data;
            }),
          fetch("/api/time-off?view=all")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.timeOffRequests = d.data;
            }),
        );
      } else if (session.role === "Manager") {
        fetches.push(
          fetch("/api/time-off?view=manager")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.timeOffRequests = d.data;
            }),
          fetch("/api/reviews?view=reviewer")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.reviews = d.data;
            }),
        );
      } else {
        fetches.push(
          fetch("/api/time-off?view=mine")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.timeOffRequests = d.data;
            }),
          fetch("/api/onboarding")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.onboarding = d.data;
            }),
          fetch("/api/reviews?view=mine")
            .then((r) => r.json())
            .then((d) => {
              if (d.success) result.reviews = d.data;
            }),
      ...
      fetches.push(
        fetch("/api/notifications")
          .then((r) => r.json())
          .then((d) => {
            if (d.success) result.notifications = d.data;
          }),
      );
  ...
  const notifications =
    (data.notifications as
      ...
      | undefined) ?? [];
  const employees =
    (data.employees as Array<{ status: string }> | undefined) ?? [];
  const payrollRuns =
    (data.payrollRuns as
      | Array<{ status: string; period: string; totalAmount: number }>
      | undefined) ?? [];
  const reviews =
    (data.reviews as
      | Array<{
  ...
  const onboardingProgress =
    onboarding && onboarding.template
      ? Math.round(
          (onboarding.completedTasks.length /
            onboarding.template.taskIds.length) *
            100,
        )
      : null;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <StatCard
                ...
                value={formatCurrency(
                  payrollRuns.find((p) => p.status === "Completed")
                    ?.totalAmount ?? 0,
                )}
                  ...
                  <div className="p-4 border-b border-[color:var(--border)]">
                    ...
                    <div className="mt-2">
                      ...
                      <p className="text-xs text-[color:var(--muted-foreground)] mt-1">
                        {onboarding.completedTasks.length} of{" "}
                        {onboarding.template?.taskIds?.length ?? 0} tasks
                        completed
                      </p>
                    </div>
                  </div>
                    ...
                    {onboarding.tasks.slice(0, 4).map((task) => {
                      ...
                      return (
                        <div key={task.id} className="flex items-start gap-3">
                          ...
                          <div>
                            <p
                              className={`text-sm ${done ? "line-through text-[color:var(--muted-foreground)]" : "font-medium"}`}
                            >
                              {task.title}
                            </p>
                            ...
                            <p className="mt-1 text-xs text-[color:var(--muted-foreground)]">
                              ...
                              {formatDate(
                                new Date(
                                  new Date(onboarding.startedAt).getTime() +
                                    task.dueOffsetDays * 86400000,
                                ).toISOString(),
                              )}
                            </p>
                            {!done && task.assignedRole === "Employee" && (
                              <button
                                type="button"
                                onClick={() => completeOnboardingTask(task.id)}
                                className="mt-1 text-xs font-semibold text-[color:var(--primary)]"
                              >
                                Mark Complete
                              </button>
                            )}
                  ...
                  <tbody>
                    {timeOffRequests.slice(0, 5).map((req, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          {formatDate(req.startDate)} &mdash;{" "}
