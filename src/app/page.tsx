...
export default function Home() {
  ...
  const stats = [
    { value: "2,400+", label: "Companies Onboarded", icon: <Users size={20} /> },
    { value: "98.9%", label: "Uptime SLA", icon: <Clock size={20} /> },
    { value: "$1.2B+", label: "Payroll Processed", icon: <DollarSign size={20} /> },
  ...
  const faqs = [
    ...
    {
      q: "Is my data safe?",
      a: "Yes. All data is encrypted in transit (TLS 1.3) and at rest (AES-256). We are SOC 2 Type II compliant and conduct regular third-party security audits.",
    },
    {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--background)] flex flex-col font-sans marketing-page-bg">
      ...
      {/* ── Hero Section ──────────────────────────────────────── */}
      <section className="relative pt-32 pb-24 px-4 hero-animated-bg" style={{ overflow: "clip" }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <ScrollReveal className="lg:col-span-7 text-left space-y-6">
              ...
              <p className="text-lg sm:text-xl text-[color:var(--muted-foreground)] leading-relaxed max-w-lg">
                PeopleCore is the unified cloud HCM platform for small to mid-size
                companies. Manage employees, payroll, time off, reviews, and org
                charts in one gorgeous interface.
              </p>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-[color:var(--foreground)]">{point}</span>
                  </div>
                ))}
              </div>

              <div className="flex flex-wrap gap-3 pt-2">
                <Link href="/signup" className="pc-btn pc-btn-primary pc-btn-lg hero-cta-btn group">
                  Start Free Trial
                  <ArrowRight size={16} className="transition-transform duration-200 group-hover:translate-x-1" />
                </Link>
                <Link href="/login" className="pc-btn pc-btn-secondary pc-btn-lg group">
      ...
      {/* ── Social Proof Stats ─────────────────────────────────── */}
      <section className="py-12 border-y border-[color:var(--border)]" style={{ background: "var(--card)" }}>
        <ScrollReveal className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
              <div key={i} className="flex flex-col items-center text-center gap-2 group stat-item">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-[color:var(--primary)] transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6"
                  style={{ background: "var(--primary-soft)" }}>
                  {stat.icon}
                </div>
                <span className="text-2xl font-extrabold text-[color:var(--foreground)] tracking-tight">{stat.value}</span>
                <span className="text-xs text-[color:var(--muted-foreground)] font-medium">{stat.label}</span>
              </div>
            ))}
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal className="max-w-7xl mx-auto space-y-14">
          <div className="text-center space-y-3">
            ...
            <p className="text-[color:var(--muted-foreground)] max-w-lg mx-auto text-base">
              Eliminate disjointed tools. Run your entire company HR operations under a
              single beautifully designed system.
            </p>
          </div>
      ...
      {/* ── How It Works ───────────────────────────────────────── */}
      <section id="how-it-works" className="py-24 px-4 border-y border-[color:var(--border)]">
        <ScrollReveal className="max-w-7xl mx-auto space-y-14">
          <div className="text-center space-y-3">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider"
              style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>
            ...
            <p className="text-[color:var(--muted-foreground)] max-w-md mx-auto">
              No lengthy implementations. No consultants required. PeopleCore gets you operational fast.
            </p>
          </div>
              ...
              <ScrollReveal key={step.step} delay={i * 80}>
                <div className="relative flex flex-col gap-4 group how-it-works-card">
                  ...
                  {i < howItWorks.length - 1 && (
                    <div className="hidden lg:block absolute top-7 left-full w-8 h-px z-10"
                      style={{ background: "var(--border)" }} />
                  )}
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-extrabold text-lg transition-all duration-300 group-hover:scale-110 group-hover:shadow-xl"
      ...
      <section className="py-24 px-4" style={{ background: "var(--muted)" }}>
        <ScrollReveal className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-6">
              ...
              {[
                ...
                <div key={idx} className="flex items-center gap-3 group">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover:scale-110"
                    ...
                    <Check size={13} className="text-[color:var(--success)]" strokeWidth={2.5} />
                  </div>
            ...
            <Link href="/signup" className="pc-btn pc-btn-primary inline-flex gap-2 group">
              Start Collaborating
      ...
      {/* ── Pricing Section ─────────────────────────────────────── */}
      <section id="pricing" className="py-24 px-4 border-t border-[color:var(--border)]">
        <ScrollReveal className="max-w-7xl mx-auto space-y-14">
          <div className="text-center space-y-3">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider"
              style={{ background: "var(--primary-soft)", color: "var(--primary)" }}>
            ...
            <p className="text-[color:var(--muted-foreground)] max-w-md mx-auto">
              No hidden platform fees. Pay only for active seats. Upgrade or downgrade anytime.
            </p>
          </div>
                ...
                >
                  ...
                  <div className="space-y-4 flex-1">
                    <div>
                      ...
                      <p className="text-xs mt-1" style={{ color: tier.primary ? "rgba(255,255,255,0.75)" : "var(--muted-foreground)" }}>
                        {tier.description}
                      </p>
                    </div>
                    <div className="flex items-baseline gap-1">
                      ...
                      {tier.price !== "Custom" && (
                        <span className="text-xs" style={{ color: tier.primary ? "rgba(255,255,255,0.7)" : "var(--muted-foreground)" }}>
                          /user/mo
                        </span>
                      )}
                    </div>
                    <ul className="space-y-2.5 pt-4 text-sm border-t" style={{ borderColor: tier.primary ? "rgba(255,255,255,0.2)" : "var(--border)" }}>
                      {tier.features.map((f) => (
                        <li key={f} className="flex items-center gap-2">
                          <Check
                            size={14}
                            className={tier.primary ? "text-white flex-shrink-0" : "text-[color:var(--primary)] flex-shrink-0"}
                            strokeWidth={2.5}
                          />
                          <span style={{ color: tier.primary ? "rgba(255,255,255,0.9)" : "var(--muted-foreground)" }}>{f}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      {/* ── Award / Trust Logos ─────────────────────────────────── */}
      <section className="py-14 px-4 border-t border-[color:var(--border)]" style={{ background: "var(--muted)" }}>
        <ScrollReveal className="max-w-5xl mx-auto text-center space-y-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-[color:var(--muted-foreground)]">
            Trusted &amp; Recognized
          </p>
          <div className="flex flex-wrap justify-center items-center gap-8 opacity-60">
            {[
              { icon: <Award size={20} />, label: "SOC 2 Type II" },
              { icon: <Shield size={20} />, label: "GDPR Compliant" },
              { icon: <HeartHandshake size={20} />, label: "99.9% SLA" },
              { icon: <Star size={20} />, label: "G2 Leader 2025" },
              { icon: <Zap size={20} />, label: "99ms p95 Latency" },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-sm font-semibold text-[color:var(--foreground)] hover:opacity-100 transition-opacity duration-200 cursor-default">
      ...
      {/* ── FAQ ─────────────────────────────────────────────────── */}
      <section id="faq" className="py-24 px-4 border-t border-[color:var(--border)]">
        <ScrollReveal className="max-w-3xl mx-auto space-y-10">
          <div className="text-center space-y-3">
            ...
            <p className="text-[color:var(--muted-foreground)]">
              Have more questions? Reach out to our team at support@peoplecore.com
            </p>
          </div>
              ...
              <ScrollReveal key={i} delay={i * 50}>
                <details className="pc-card p-5 faq-item group">
                  <summary className="flex items-center justify-between cursor-pointer list-none gap-4">
                    ...
                    <ChevronDown size={16} className="text-[color:var(--muted-foreground)] flex-shrink-0 faq-chevron transition-transform duration-300" />
                  </summary>
      ...
      <section className="py-20 px-4 border-t border-[color:var(--border)] cta-banner-section">
        <ScrollReveal className="max-w-3xl mx-auto text-center space-y-6">
          ...
          <p className="text-base text-white/80 leading-relaxed">
            Join 2,400+ companies that trust PeopleCore to manage their most important asset — their people.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/signup" className="pc-btn pc-btn-lg cta-banner-btn group">
              Start Free Trial
              <ArrowRight size={16} className="transition-transform duration-200 group-hover:translate-x-1" />
            </Link>
            <Link href="/login" className="pc-btn pc-btn-lg cta-banner-btn-secondary">
              View Live Demo
            </Link>
          </div>
      ...
      <footer className="mt-auto py-12 bg-[color:var(--sidebar)] text-[color:var(--sidebar-foreground)]">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 pb-10 border-b" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
            <div className="space-y-4">
              <Logo variant="dark-horizontal-short" size="md" priority />
              <p className="text-xs text-[color:var(--sidebar-muted)] leading-relaxed">
                The unified cloud HCM platform for modern companies. Beautiful, powerful, and built for your team.
              </p>
            </div>
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-[color:var(--sidebar-muted)]">Product</p>
              {[
                ...
                <Link key={link.href} href={link.href} className="block text-xs text-[color:var(--sidebar-muted)] hover:text-[color:var(--sidebar-foreground)] hover:translate-x-0.5 transition-all duration-150">
                  {link.label}
                </Link>
              ))}
            </div>
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-[color:var(--sidebar-muted)]">Platform</p>
              {[
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-[color:var(--sidebar-muted)]">Legal</p>
              {[
          ...
          <div className="pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-[color:var(--sidebar-muted)]">
            ...
            <div className="flex gap-4">
              <Link href="/privacy-policy" className="hover:text-[color:var(--sidebar-foreground)] hover:underline transition-colors">Privacy</Link>
              <Link href="/terms" className="hover:text-[color:var(--sidebar-foreground)] hover:underline transition-colors">Terms</Link>
            </div>
          </div>
        </div>
      </footer>
